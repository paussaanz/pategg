...
$formSteps = [
	...
	[
		'id'     => 'tercerPasoProfesionales',
		'title'  => __('¿Cuál es tu perfil profesional?', 'pategg'),
		'fields' => [
			[
				'type'    => 'radio',
				'name' => 'perfilProfesional',
				'options' => [
					'arquitecto'   => __('Arquitecto', 'pategg'),
					'constructor'  => __('Constructor', 'pategg'),
					'interiorista' => __('Interiorista', 'pategg'),
					'otros'        => __('Otros', 'pategg')
				]
			]
		],
		'button' => __('Siguiente', 'pategg')
	...
	[
		'id'     => 'sextoPasoProfesionales',
		'title'  => __('¿Qué producto te interesa?', 'pategg'),
		'fields' => [
			[
				'type'    => 'radio',
				'name' => 'productoInteres',
				'options' => [
					'ascensor'            => __('Ascensor', 'pategg'),
					'plataformaElevadora' => __('Plataforma elevadora', 'pategg'),
					'bespoke'             => __('Bespoke, lo quiero a medida', 'pategg'),
					'noClaro'             => __('No lo tengo claro', 'pategg')
				]
			]
		],
		'button' => __('Siguiente', 'pategg')
	...
	[
		'id'     => 'novenoPasoProfesionales',
		'title'  => __('¿Tienes planos o imágenes del proyecto?', 'pategg'),
		'fields' => [
			[
				'type'    => 'radio',
				'name' => 'documentos',
...
?>
<div class="formularioPresupuestos h-100" id="formularioPresupuestosProfesionales">
			...
			<div class="col-5 d-none d-md-block">
				<div class="row justify-content-center mb-3">
					<div class="col-12 d-flex align-items-center icons-black">
						<button type="button"
								class="btn btn-link ps-4 text-dark prev-step prevButtonPresupuestoProfesionales icon-left text-decoration-none opacity-0">
							<?= __('Atras', 'pategg') ?>
						</button>
					</div>
                    <div class="col-10">
			...
			<section class="container-fluid p-0">
                <div class="row g-0 ">
					...
					<div class="col-12 col-md-7">
						<form id="formPresupuestosProfesionales" class="bg-warm-gray p-4 legend m-0 h-100">
							<?php foreach ($formSteps as $key => $step) :
								$paso = $key + 1;
								?>
								<div id="<?= $step['id'] ?>" class="h-100 step-content-PE <?= $paso === 1 ? '' : 'd-none' ?>">
									<div class="row h-100 align-content-between">
													...
													<?php foreach ($field['options'] as $value => $label) : ?>
														<div class="form-check mb-3">
															<input class="form-check-input"
																   type="radio"
																   name="<?= $field['name'] ?>"
																   id="<?= $field['name'] . '_PE' . '_' . $value ?>"
																   value="<?= $value ?>">
															<label class="form-check-label"
																   for="<?= $field['name'] . '_PE' . '_' . $value ?>"><?= $label ?></label>
														</div>
													<?php endforeach; ?>
												...
												<?php elseif ($field['type'] === 'checkbox') : ?>
													<div class="form-check mb-3">
														<input class="form-check-input"
															   ...
															   name="<?= $field['name'] ?>"
															   id="<?= $field['name'] . '_PE' ?>">
														<label class="form-check-label" for="<?= $field['name'] . '_PE' ?>"><?= $field['label'] ?></label>
													</div>
												<?php elseif ($field['type'] === 'checkbox-privacy') : ?>
													<div class="form-check mb-3">
														<input class="form-check-input"
															   ...
															   name="<?= $field['name'] ?>"
															   id="<?= $field['name'] . '_PE' ?>">
														<label class="form-check-label" for="<?= $field['name'] . '_PE' ?>">
															<?= __('Acepto la', 'pategg') ?>
										...
										<div class="col-12">
											<button type="button"
													class="btn btn-primary w-100 nextButtonPresupuestoProfesionales"
